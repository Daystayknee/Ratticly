const STORAGE_KEY = 'ratticly-store-v3';
...
const api = {
  async latency(){ await new Promise(r=>setTimeout(r,80)); },
...
function computeMilestones(r){
  const d=daysOld(r.dob);
  const m=[];
  if(d>=21 && d<35) m.push('weaning window');
  if(d>=56 && d<84) m.push('breeding-age check');
  if(d>=540 && d<700) m.push('retirement planning');
  const recentWeight=db.data.weights.filter(w=>w.ratId===r.id).sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,2);
  if(recentWeight.length===2 && Math.abs(recentWeight[0].grams-recentWeight[1].grams)>=30) m.push('weight shift alert');
  if(r.health==='Watchlist') m.push('health watch');
  return m;
}

function computeStats(){
  const rats=db.data.rats;
  return [['Total Rats',rats.length],['Bucks',rats.filter(r=>r.sex==='Buck').length],['Does',rats.filter(r=>r.sex==='Doe').length],['Pregnant',rats.filter(r=>r.status==='Pregnant').length],['Litters',db.data.litters.length],['Pairings',db.data.pairings.length],['Health',db.data.healthRecords.length],['Tasks',db.data.tasks.length],['Inventory Alerts',db.data.inventory.filter(i=>i.qty<=i.reorder).length],['Reservations',db.data.reservations.length],['Events',db.data.events.length],['Milestone Alerts',rats.filter(r=>computeMilestones(r).length).length]];
}
...
function renderFeaturedRat(){
  const wrap=$('#featured-rat'); const featured=db.data.rats[0]; if(!wrap||!featured) return;
  wrap.innerHTML=`<img class="hero-photo" src="${featured.img}" alt="${featured.name}"/><div class="hero-meta"><p class="badge">🌟 Featured Rat</p><h3>${featured.name} <span class="badge">${featured.status}</span></h3><p class="hero-note">${featured.call} · ${featured.reg}</p><div class="hero-stats"><span class="badge">${featured.sex}</span><span class="badge">${featured.variety}</span><span class="badge">${featured.weight}g</span><span class="badge">Line: ${featured.line}</span><span class="badge">Genotype: ${featured.genotype||'n/a'}</span></div><p class="hero-note">${featured.notes||'No notes yet.'}</p></div>`;
}
function renderDailyFocus(){ const ul=$('#daily-focus'); ul.innerHTML=dailyFocusItems.map(i=>`<li>${i}</li>`).join(''); }
function renderMilestones(){
  const ul=$('#milestone-watch'); ul.innerHTML='';
  db.data.rats.forEach(r=>{ const ms=computeMilestones(r); if(ms.length) ul.appendChild(make('li',`<strong>${r.name}</strong>: ${ms.join(', ')}`)); });
  if(!ul.children.length) ul.innerHTML='<li>No active milestone alerts today.</li>';
}
...
function renderRats(){
  ...
  filteredRats().forEach(r=>{
    const milestones=computeMilestones(r);
    const card=make('article',`<img src="${r.img}" alt="${r.name}"/><div class="rat-head"><h3>${r.name}</h3><span class="badge">${r.status}</span></div><p class="rat-sub">${r.call} • ${r.reg}</p><p class="rat-sub">${r.sex} • ${r.variety} • DOB ${r.dob}</p><p class="rat-sub">Cage ${r.cage} • ${r.weight}g • ${r.line}</p><div class="rat-tags"><span class="badge">${r.breed}</span><span class="badge ${r.health==='Watchlist'?'warn':'good'}">${r.health}</span><span class="badge">${r.availability}</span>${milestones.length?`<span class="badge warn">${milestones[0]}</span>`:''}</div>`,'card rat-card');
...
function runGenetics(){ const a=parseGenotype($('#geno-a').value), b=parseGenotype($('#geno-b').value); const loci=[...new Set([...Object.keys(a),...Object.keys(b)])]; const out=$('#genetics-results'); out.innerHTML=''; loci.forEach(l=>{ if(!a[l]||!b[l]) return out.appendChild(make('li',`${l}: insufficient data`)); out.appendChild(make('li',`${l}: ${locusOdds(a[l],b[l]).map(o=>`${o.g} ${o.pct}%`).join(' · ')}`)); }); }
function sharedAncestors(aId,bId,depth=3){ const anc=(id,d,set=new Set())=>{ if(!id||d===0) return set; const r=byId('rats',id); if(!r) return set; [r.sireId,r.damId].forEach(pid=>{ if(pid){ set.add(pid); anc(pid,d-1,set);} }); return set; }; const a=anc(aId,depth), b=anc(bId,depth); return [...a].filter(x=>b.has(x)); }
...
function ancestorsToDepth(rootId, depth){
  const nodes=[], links=[];
  function walk(id,level,xHint){ if(!id||level>depth) return; const r=byId('rats',id); if(!r) return; nodes.push({id:r.id,name:r.name,level,health:r.health,traits:r.traits||[],xHint}); [r.sireId,r.damId].forEach((pid,i)=>{ if(pid){ links.push({from:pid,to:r.id}); walk(pid,level+1,xHint+(i?1:-1)*(220/level)); }}); }
  walk(rootId,1,480); return {nodes,links};
}
function renderLineageGraph(){
  const svg=$('#lineage-graph'); if(!svg) return;
  const depth=Number($('#lineage-depth').value), showHealth=$('#overlay-health').checked, showTraits=$('#overlay-traits').checked;
  const root=state.currentRatId || db.data.rats[0]?.id; if(!root){ svg.innerHTML=''; return; }
  const {nodes,links}=ancestorsToDepth(root, depth); svg.innerHTML=''; const pos={};
  nodes.forEach(n=>{ pos[n.id]={x:Math.max(70,Math.min(890,n.xHint)), y:60+(n.level-1)*85}; });
  links.forEach(l=>{ const a=pos[l.from], b=pos[l.to]; if(!a||!b) return; const line=document.createElementNS('http://www.w3.org/2000/svg','line'); line.setAttribute('x1',a.x); line.setAttribute('y1',a.y); line.setAttribute('x2',b.x); line.setAttribute('y2',b.y); svg.appendChild(line); });
  nodes.forEach(n=>{ const g=document.createElementNS('http://www.w3.org/2000/svg','g'); g.style.cursor='pointer'; const c=document.createElementNS('http://www.w3.org/2000/svg','circle'); c.setAttribute('cx',pos[n.id].x); c.setAttribute('cy',pos[n.id].y); c.setAttribute('r',22); c.setAttribute('fill',showHealth&&n.health==='Watchlist'?'#ffd7df':'#eaf0ff'); c.setAttribute('stroke','#8ea0d8'); const t=document.createElementNS('http://www.w3.org/2000/svg','text'); t.setAttribute('x',pos[n.id].x); t.setAttribute('y',pos[n.id].y+4); t.setAttribute('text-anchor','middle'); t.setAttribute('font-size','10'); t.textContent=n.name; g.append(c,t); if(showTraits){ const tx=document.createElementNS('http://www.w3.org/2000/svg','text'); tx.setAttribute('x',pos[n.id].x); tx.setAttribute('y',pos[n.id].y+34); tx.setAttribute('text-anchor','middle'); tx.setAttribute('font-size','9'); tx.textContent=(n.traits||[]).slice(0,2).join(','); g.append(tx);} g.addEventListener('click',()=>openRatDrawer(n.id)); svg.appendChild(g); });
}

function renderLists(){ $('#health-list').innerHTML=db.data.healthRecords.map(h=>`<li>${ratName(h.ratId)} — ${h.diagnosis} (${h.date})</li>`).join(''); $('#inventory-list').innerHTML=db.data.inventory.map(i=>`<li>${i.name}: ${i.qty} ${i.unit} (reorder ${i.reorder})</li>`).join(''); $('#task-list').innerHTML=db.data.tasks.map(t=>`<li>${t.label} — ${t.due}</li>`).join(''); }

function openRatDrawer(id){
  state.currentRatId=id;
  state.currentProfileTab='Overview';
  const drawer=$('#rat-drawer');
...
function drawerTabContent(r){
  const litters=db.data.litters.filter(l=>l.sireId===r.id||l.damId===r.id), weights=db.data.weights.filter(w=>w.ratId===r.id), health=db.data.healthRecords.filter(h=>h.ratId===r.id);
  return {
    Overview:`<p><strong>${r.name}</strong> · ${r.sex} · ${r.variety}</p><p>${r.notes||''}</p><p><strong>Milestones:</strong> ${computeMilestones(r).join(', ') || 'none'}</p>`,
    ...
    Photos:`<img src="${r.img}" alt="${r.name}" style="width:100%;border-radius:12px;aspect-ratio:4/3;object-fit:cover;"><div class="field"><label>Upload memories (multiple images)</label><input id="photo-files" type="file" accept="image/*" multiple><button class="chip" id="upload-photos" type="button">Upload & Save</button></div><div class="gallery">${(r.photos||[]).map((p,i)=>`<figure><img src="${p.src}" alt="${r.name} memory ${i+1}"><figcaption>${p.label||('Memory '+(i+1))}</figcaption></figure>`).join('')}</div>`,
    ...
    Timeline:`<label>Filter: <select id="timeline-filter"><option value="all">All</option><option value="weight">Weight</option><option value="health">Health</option><option value="rat">Rat changes</option><option value="media">Media</option></select></label><ul id="timeline-list" class="timeline"></ul>`
  }[state.currentProfileTab] || '<p>Empty.</p>';
}
function renderTimelineFiltered(r){
  const filter=$('#timeline-filter')?.value||'all'; const ul=$('#timeline-list'); if(!ul) return;
  let events=db.data.events.filter(e=>e.entityId===r.id || (e.type==='rat.media' && e.entityId===r.id));
  ...
  if(filter==='media') events=events.filter(e=>e.type.includes('media'));
  ul.innerHTML=events.slice(0,30).map(e=>`<li>${e.type} — ${e.summary} <span class="rat-sub">${date(e.at)}</span></li>`).join('');
}
async function uploadRatPhotos(r){
  const input=$('#photo-files'); if(!input?.files?.length) return;
  const files=[...input.files].slice(0,10);
  const encoded=await Promise.all(files.map((file)=>new Promise((res,rej)=>{ const fr=new FileReader(); fr.onload=()=>res({src:fr.result,label:`${new Date().toISOString().slice(0,10)} · ${file.name}`}); fr.onerror=rej; fr.readAsDataURL(file);} )));
  const photos=[...(r.photos||[]), ...encoded];
  await api.update('rats',r.id,{photos});
  db.addEvent('rat.media',`Uploaded ${encoded.length} photo(s) for ${r.name}`,'rat',r.id);
  rerender();
}
function renderDrawer(){
  ...
  $('#upload-photos')?.addEventListener('click',async()=>{ try{ await uploadRatPhotos(r); }catch(err){ alert(err.message); } });
...
async function submitForm(kind,data,editId){
  if(kind==='rat'){
    const current=byId('rats',editId);
    const payload={ ...(current||{}), id:editId||uid('r'), name:data.name, call:data.name, reg:data.name, sex:data.sex, dob:data.dob, status:data.status, variety:data.variety, genotype:data.genotype||'', breed:current?.breed||'Pending', cage:current?.cage||'Unassigned', weight:current?.weight||0, health:current?.health||'Clear', availability:current?.availability||'Available', line:current?.line||'Unassigned', traits:current?.traits||[], carriers:current?.carriers||[], sireId:current?.sireId||null, damId:current?.damId||null, temperament:current?.temperament||'', notes:current?.notes||'', photos:current?.photos||[], img:current?.img||'https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?auto=format&fit=crop&w=900&q=80' };
...
function rerender(){ renderAuth(); renderFeaturedRat(); renderDailyFocus(); renderMilestones(); renderStats(); renderEvents(); renderQuickAdd(); renderRats(); renderLitters(); renderPairings(); renderLineageGraph(); renderLists(); if(state.currentRatId) renderDrawer(); }
...
function wire(){
  ...
  $('#quick-add-form').addEventListener('submit',async e=>{
    e.preventDefault();
    const kind=e.currentTarget.dataset.kind, editId=e.currentTarget.dataset.editId||null;
    const data=Object.fromEntries(new FormData(e.currentTarget).entries());
    const err=validate(kind,data); if(err){ $('#form-error').textContent=err; return; }
    let optimisticId=null;
    try{
      if(!editId && kind==='rat'){
        optimisticId=uid('rtmp');
        db.data.rats.push({ id:optimisticId, name:data.name, call:data.name, reg:data.name, sex:data.sex, dob:data.dob, status:data.status, variety:data.variety, genotype:data.genotype||'', breed:'Pending', cage:'Unassigned', weight:0, health:'Clear', availability:'Available', line:'Unassigned', traits:[], carriers:[], sireId:null, damId:null, temperament:'', notes:'', photos:[], img:'https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?auto=format&fit=crop&w=900&q=80' });
        rerender();
      }
      await submitForm(kind,data,editId);
      if(optimisticId) db.data.rats=db.data.rats.filter(r=>r.id!==optimisticId);
      $('#quick-add-modal').close(); rerender();
    }catch(ex){ if(optimisticId) db.data.rats=db.data.rats.filter(r=>r.id!==optimisticId); $('#form-error').textContent=ex.message; rerender(); }
  });
