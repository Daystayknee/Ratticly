...
function renderGenericLists(){
  ...
  const mapPlacement=(r,next)=>`<li><span class="placement-meta"><span class="badge sale-tag ${saleTagClass(r.availability)}">${r.availability}</span> <strong>${r.name}</strong> · ${r.variety} · ${r.sex}</span><span class="placement-actions"><button class="tiny-btn open-rat" data-id="${r.id}">Profile</button><button class="tiny-btn placement-move" data-id="${r.id}" data-next="${next}">Move to ${next}</button></span></li>`;
  ...
  $('#settings-list').innerHTML=[`Current user role: ${db.currentUser().role}`,`Theme: ${document.body.dataset.theme}`,`Storage key: ${STORAGE_KEY}`,`Rats tracked: ${db.data.rats.length}`,`Events stored: ${db.data.events.length}`].map(x=>`<li>${x}</li>`).join('');
}
...
async function uploadRatPhotos(r){ const input=$('#photo-files'); if(!input?.files?.length) return; const files=[...input.files].slice(0,15); const encoded=await Promise.all(files.map(file=>new Promise((res,rej)=>{ const fr=new FileReader(); fr.onload=()=>res({src:fr.result,label:`${new Date().toISOString().slice(0,10)} · ${file.name}`}); fr.onerror=rej; fr.readAsDataURL(file);}))); const photos=[...(r.photos||[]), ...encoded]; await api.update('rats',r.id,{photos}); db.addEvent('rat.media',`Uploaded ${encoded.length} photo(s) for ${r.name}`,'rat',r.id); rerender(); }
async function setRatAvailability(id,nextStatus){ const rat=byId('rats',id); if(!rat) return; const before=rat.availability||'Available'; try{ await api.update('rats',id,{availability:nextStatus}); db.addEvent('rat.placement.updated',`Placement changed for ${rat.name}: ${before} → ${nextStatus}`,'rat',id); rerender(); }catch(e){ alert(e.message); } }
...
async function submitForm(kind,data,editId){
  ...
  if(kind==='rat'){
    ...
    const payload={...(cur||{}),id:editId||uid('r'),name:data.name,call:data.name,reg:data.name,sex:data.sex,dob:data.dob,status:data.status,variety:data.variety,genotype:data.genotype||'',breed:cur?.breed||'Pending',cage:cur?.cage||'Unassigned',weight:cur?.weight||0,health:cur?.health||'Clear',availability:cur?.availability||'Available',line:cur?.line||'Unassigned',traits:cur?.traits||[],carriers:cur?.carriers||[],sireId:cur?.sireId||null,damId:cur?.damId||null,temperament:cur?.temperament||'',notes:cur?.notes||'',photos:cur?.photos||[],img:cur?.img||'https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?auto=format&fit=crop&w=900&q=80'};
  ...
  if(kind==='reservation') await push('reservations',{id:uid('rs'),...data},`Reservation for ${data.adopter}`,'reservation');
...
function exportJson(){ const blob=new Blob([JSON.stringify(db.data,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`ratticly-backup-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(a.href); }
function importJson(file){ const fr=new FileReader(); fr.onload=()=>{ try{ const obj=JSON.parse(fr.result); db.data=obj; db.persist(); db.addEvent('system.import','Imported backup JSON','system'); rerender(); }catch(e){ alert('Invalid JSON backup'); } }; fr.readAsText(file); }
...
function wire(){
  ...
  $('#quick-add-form').addEventListener('submit',async e=>{ e.preventDefault(); const kind=e.currentTarget.dataset.kind, editId=e.currentTarget.dataset.editId||null; const data=Object.fromEntries(new FormData(e.currentTarget).entries()); const err=validate(kind,data); if(err){ $('#form-error').textContent=err; return; } let optimisticId=null; try{ if(!editId&&kind==='rat'){ optimisticId=uid('rtmp'); db.data.rats.push({id:optimisticId,name:data.name,call:data.name,reg:data.name,sex:data.sex,dob:data.dob,status:data.status,variety:data.variety,genotype:data.genotype||'',breed:'Pending',cage:'Unassigned',weight:0,health:'Clear',availability:'Available',line:'Unassigned',traits:[],carriers:[],sireId:null,damId:null,temperament:'',notes:'',photos:[],img:'https://images.unsplash.com/photo-1585110396000-c9ffd4e4b308?auto=format&fit=crop&w=900&q=80'}); rerender(); }
      await submitForm(kind,data,editId); if(optimisticId) db.data.rats=db.data.rats.filter(r=>r.id!==optimisticId); $('#quick-add-modal').close(); rerender();
